    ...
    if mode == 'train':
        ...
        for episode in range(max_episodes):
            ...
            # for n in name:
                # print(type(state[n]))
                # print(state[n].shape)
            lidar = np.concatenate((state['goal_dist'], state['goal_compass'], state['pillars_lidar']), axis=0)
            state = [lidar]
            for step in range(max_episode_steps):
                ...
                next_lidar = np.concatenate((next_state['goal_dist'], next_state['goal_compass'], next_state['pillars_lidar']), axis=0)
